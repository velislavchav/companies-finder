...
function roundMonthDiff(d1) {
  let monthsCount = monthDiff(d1);
  let result = "0 months";
  if (monthsCount > 0 && monthsCount < 12) {
    result = `${monthsCount} months`
  } else if (monthsCount >= 12) {
    let monthsToYearRounded = Math.round(monthsCount / 12);
    result = `${monthsToYearRounded} years`
  }
      ...
      person?.functions?.forEach((func) => {
        returnValue.push({
          ...
          seniority: func?.valid_from ? (
            <div>
              <div>{roundMonthDiff(new Date(func?.valid_from))}</div>
              <div
                style={{
